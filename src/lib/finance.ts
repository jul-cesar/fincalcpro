...
import { z } from "zod";
import type { AmortizationMethod, AnnuityTiming } from "../types";
...
export function annuityPayment(
  ...
  const factor = i.isZero() ? D(count).mul(timingFactor) : D(1).plus(i).pow(count).minus(1).div(i).mul(timingFactor);
...
export function buildAnnuityRows(paymentValue: number, ratePercent: number, periods: number, timing: AnnuityTiming) {
  let balance = D(0);
...
export function amortization(method: AmortizationMethod, loan: number, ratePercent: number, periods: number) {
  ...
  for (let period = 1; period <= count; period += 1) {
    ...
    if (method === "german") {
      ...
      payment = i.isZero() ? principal.div(count) : principal.mul(i.div(D(1).minus(D(1).plus(i).pow(-count))));
...
export function netPresentValue(flows: number[], rateDecimal: number) {
  ...
  return flows.reduce((total, flow, index) => total.plus(D(flow).div(D(1).plus(i).pow(index))), D(0)).toNumber();
}
...
export function cashflowSeries(initial: number, payment: number, periods: number, growthPercent: number) {
  ...
  for (let period = 1; period <= Math.max(1, Math.min(24, periods)); period += 1) {
